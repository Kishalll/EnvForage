...
FORBIDDEN_PATTERNS: list[tuple[str, str]] = [
    ...
    (r"curl\s+(?:-[a-zA-Z]*?[sS]+?[a-zA-Z]*?\s+)?https?://(?!(?:micro\.mamba\.pm|astral\.sh)/)\S+\s*\|\s*(?:ba)?sh", "Curl-pipe-to-shell (untrusted exec)"),
    (r"wget\s+(?:-[a-zA-Z]*?[sS]+?[a-zA-Z]*?\s+)?https?://(?!(?:micro\.mamba\.pm|astral\.sh)/)\S+\s*\|\s*(?:ba)?sh", "Wget-pipe-to-shell (untrusted exec)"),
    (r"wget\s+[^;\|&]+?(-O\s+\S+).*(?:&&|;|\||\|\||\n)\s*(?:ba)?sh\s+\1", "Wget download-and-execute pattern (sequential/chained)"),
    (r"wget\s+[^;\|&]+?(-O\s+(\S+)).*(?:&&|;|\||\|\||\n)\s*(?:ba)?sh\s+\2", "Wget download-and-execute pattern (explicit target)"),
    (r"curl\s+[^;\|&]*?(-o|--output)\s+(\S+).*(?:&&|;|\||\|\||\n)\s*(?:ba)?sh\s+\2", "Curl download-and-execute pattern"),
    (r"curl\s+[^;\|&]*?(-O|--remote-name)\s+.*(?:&&|;|\||\|\||\n)\s*(?:ba)?sh\s+", "Curl remote-name download-and-execute pattern"),
    (r"curl\s+[^;\|&]*?>\s*(\S+).*(?:&&|;|\||\|\||\n)\s*(?:ba)?sh\s+\1", "Curl redirect download-and-execute pattern"),
    (r"(?:iex|Invoke-Expression)\s*\(?\s*(?:iwr|Invoke-WebRequest|Invoke-RestMethod|curl|wget)\s+(?!https?://astral\.sh/)", "PowerShell malicious download cradle"),
    (r"(?:iwr|Invoke-WebRequest|Invoke-RestMethod|curl|wget)\s+(?!https?://astral\.sh/)\S+.*\s*\|\s*(?:iex|Invoke-Expression)", "PowerShell piped download cradle"),
    (r"(?:iex|Invoke-Expression)\s*\(?\s*(?:\(?(?:New-Object)\s+Net\.WebClient\)?\.(?:DownloadString|DownloadFile))\s*\(", "PowerShell .Net WebClient download cradle"),
